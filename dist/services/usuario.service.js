...
export class UsuarioService {
    ...
    async crearUsuario(nombre, apellido, email, direccion, dni, password) {
        if (!dni || dni.length < 7) {
    ...
    async login(email, password) {
        ...
        const token = jwt.sign({ userId: user.id, tipoUsuarioId: user.tipoUsuarioId }, process.env.JWT_SECRET || 'secret', { expiresIn: '8h' });
